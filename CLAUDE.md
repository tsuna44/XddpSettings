...
`change`: 既存コードへの変更（通常の CR）。工程04（スペックアウト）を実行する。
...
| xddp.07.code | 工程9（コーディング）+ 工程10（静的検証）※工程10は xddp.08.verify でも単体実行可 |
| xddp.08.verify | 工程10（静的検証）— 人が手動実行する場合（xddp.07.code の自動検証と同一内容） |
| xddp.09.test | 工程11（テスト設計）+ 工程12（テスト実行）+ 工程13（不具合修正）+ 工程14（不具合フィードバック） |
| xddp.10.specs | 工程15（最新仕様書作成） |
| xddp.close | CR クローズ（気づき集約・知見ログ更新） |
| xddp.update-knowledge | 工程対象外・随時実行 |
| xddp.review | 単体AIレビュー（工程対象外・随時実行） |
| xddp.plan-review | プランAIレビュー→修正ループ（工程対象外・随時実行） |
...
| xddp.sync-design | コード→DSN再生成・リビジョン追加（工程対象外・随時実行） |
| xddp.codemap | モジュールカタログ生成・更新（工程対象外・随時実行） |
