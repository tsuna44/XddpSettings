...
> - 認証情報・APIキーは絶対に記載しない
> - 未記入セクションは `/xddp.fill-rulebook notify-svc` を実行することで CR の記述内容をもとに自動ドラフトできます
...
### フィルタ条件の暗黙ルール
```python
# 「空＝制限なし」という暗黙ルールを持つフィルタ条件は、コード上のコメントと
# テストケースの両方で明示しないと意図が伝わらない。
# 例: NotificationTarget.label_filter が空配列の場合は「全件対象」として扱う。
# 設計書（CHD）にも必ず明記すること。
# （出典: LL-002, CR-2026-900）
```

（このリポジトリ固有の実装パターンをここに記述する）
...
# 「空＝制限なし」等の暗黙ルールを持つフィールドは、対応するテストケースを必ず1件以上用意する
# （出典: LL-002, CR-2026-900）
```
...
| 日付 | 更新CR | 更新内容 |
|------|--------|---------|
| 2026-06-21 | CR-2026-900 | 初期作成。Section 4 にフィルタ条件の暗黙ルール・テストパターンを追記（LL-002） |
