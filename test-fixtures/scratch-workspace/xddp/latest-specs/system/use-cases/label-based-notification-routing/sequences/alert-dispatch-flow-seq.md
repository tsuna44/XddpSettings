...
デバイス側でアラート条件（閾値超過等）が発生すると、device-svc.alert-dispatcher がラベル一覧を含む `device.alert.raised` イベントを生成し、EventBus を介して notify-svc.notifier に配信する。notifier はイベントのラベルと subscription-manager に登録された通知先ラベルを照合し、合致する通知先にのみ通知を送信する（CR-2026-900-UR-003：ラベル単位の通知振り分け）。

本シナリオの起動主体は人間の操作ではなく、デバイスの異常検知（アラート発生）である。CRS CR-2026-900-UR-003 にはこの配信フロー自体の起動主体に関する明示記述がないため、SPO のエントリポイント種別（device-svc.alert-dispatcher が発行元）から ExternalSystem（デバイス側アラート発生）として推定補完した。
...
> 参加者スコープ: アクター〜システム境界。
> 本シナリオは CR-2026-900-UR-003 にアクター・トリガーの明示記述がないため、SPO §3（cross SPO・notifier モジュール SPO）のイベント発行元から起動主体を推定補完した（「（推定補完）」）。
...
| バージョン | CR | 日付 | 変更内容 |
|---|---|---|---|
| 1.0.0 | CR-2026-900 | 2026-06-21 | 初版作成（cross SPO §3 + notify-svc/notifier モジュール SPO §3 から AI 合成。起動主体は CR-2026-900-UR-003 に明示記述なし・SPO エントリポイントから推定補完） |
