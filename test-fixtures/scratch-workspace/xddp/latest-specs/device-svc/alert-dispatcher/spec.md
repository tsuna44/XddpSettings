...
`device.alert.raised` イベントに、device-registry から取得したラベル一覧を含める（CR-2026-900-SP-004）。
...
- アラート一覧はラベル指定でフィルタできる（CR-2026-900-SP-005）。
...
| # | 種別 | 内容 | 対応方針 |
|---|------|------|----------|
| 1 | 修正点／改善案／懸念／質問 | CR-2026-900-SP-005（アラート一覧のラベル絞り込み）の CHD 詳細設計（§3.x）が見つからず、CRS §4 の After 記述のみで本仕様書を作成した | 次回CR |
...
| バージョン | CR | 日付 | 変更内容 |
|---|---|---|---|
| 1.0.0 | CR-2026-900 | 2026-06-21 | 初版作成（SPO から生成）。CR-2026-900-SP-004 反映: AlertEvent に labels フィールドを追加し device-registry から取得したラベルを設定（Before: device_id, severity, raised_at のみ → After: labels を追加）。CR-2026-900-SP-005 反映: アラート一覧のラベル絞り込み機能を追加（Before: ラベルで絞り込めない → After: ラベル指定でフィルタ可能） |
