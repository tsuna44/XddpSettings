...
> - 単一リポジトリ内の規約は各 `project-rulebook-{repo}.md` に委ねる
> - プロジェクト全体の規約（チーム・開発プロセス等）は `project-rulebook.md`（共通）に委ねる
>
> **更新ルール:**
...
> - 認証情報・APIキーは絶対に記載しない
> - 未記入セクションは `/xddp.fill-rulebook cross` を実行することで CR の記述内容をもとに自動ドラフトできます
...
```
1. 変更する CR の CRS-{CR}.md に「インタフェース変更内容」を記載する
...
# CR-2026-900 の知見（LL-003）:
# 複数リポジトリで共有するイベント・データ型に breaking 変更を加える場合、
# CHD のインタフェース変更サマリに breaking フラグを立てるだけでなく、
# デプロイ順序・移行手順まで明記しないと運用時に問題が起こり得る。
# cross の CHD で breaking: true とした項目は、デプロイ順序または
# 後方互換措置（フィールドのoptional化等）を必ず記載する。
# （出典: LL-003, CR-2026-900）
```
...
| 日付 | 更新CR | 更新内容 |
|------|--------|---------|
| 2026-06-21 | CR-2026-900 | 初期作成。§6 にイベント型 breaking 変更時のデプロイ順序明記ルールを追記（LL-003） |
