...
- 分割実行継続マーカーをまたぐ複数セッション実行（うち「AI_INDEX先行更新フラグの鮮度」サブシナリオのみ
  `test-fixtures/scratch-workspace-split-flag/` で別途確認済み。RESUME_UR_LIST等の伝達自体は本項目のまま未確認）
- 並行CR競合シナリオ（Step C0-4: OVERLAP_FILES）— `baseline_docs` をgit管理下に置けば再現可能

シングルリポジトリ構成（IS_MULTI=false）は `test-fixtures/scratch-workspace-single-repo/` で別途確認済み。
