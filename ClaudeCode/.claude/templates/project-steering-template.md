...
> - 認証情報・APIキーは絶対に記載しない
> - 未記入セクションは `/xddp.fill-steering` を実行することで CR の記述内容をもとに自動ドラフトできます
...
| リポジトリ名 | 役割 | 主要技術 |
|------------|------|---------|
| （例）tasksaas-api | REST API サーバー | FastAPI, SQLAlchemy |
...
> 以下はプロジェクト横断のデフォルト原則。プロジェクト固有の規約はこの下に追記する。

```
# ── デフォルト原則（全プロジェクト共通） ──
# 1. エラーを握り潰さない: catch/except 後は必ずログ出力か再スローする
# 2. フェイルファスト: 無効な入力は処理開始前に検証し、早期リターンする
# 3. 外部境界でのみキャッチ: 内部関数はエラーを上位に伝播させる
# 4. ユーザー向けメッセージと技術詳細を分離する（内部エラーをそのまま露出しない）
# 5. 外部API・I/O呼び出しは必ず例外処理を行い、タイムアウトを設定する
# 6. ログはスタックトレース付きで出力する（エラー発生箇所を追跡可能にする）
# 7. 一時的エラー（タイムアウト・ネットワーク）はリトライ可、永続的エラー（4xx）はリトライ不可
```

（プロジェクト固有のエラーハンドリング規約をここに追記する）
