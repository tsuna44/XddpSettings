...
DFD（Section 4.2）は外部副作用の有無に関わらず常に必須生成する（Step 10 で処理）。
...
10. 観察結果の集約（SPO サマリー Section 4.1 / 4.2 / 5.6 への書き込み）:
    ...
    **前処理（ファイル存在確認）:**
    `{OUTPUT_DIR}/_observation-memo.md` が存在しない場合（Steps 2〜4 の観察が一切行われなかった場合）:
      ...
      `{SIDE_EFFECTS_DFD_PLACEHOLDER}` を以下の Mermaid DFD で Edit 置換する:
        「外部呼び出し元（詳細未調査）」→「変更対象プロセス（Wave 0 シンボル名）」→「呼び出し元へ返却（詳細未調査）」
        Wave 0 シンボル名の取得元: `{OUTPUT_DIR}/discovery-log.md` の「Wave 0」エントリから識別子を取得する。
          `discovery-log.md` も存在しない場合は CRS ファイルの SP 項目の「変更対象識別子」列から取得する。
          どちらも参照できない場合は「変更対象プロセス（識別子未特定）」とする。
        ※ 観察が一切行われなかった場合の最小 DFD。調査未完了であることをアーキテクトに伝える。
    ...
    **冪等性チェック（再実行時）:**
    ...
    Section 4.2 DFD の後処理（Section 4.1 書き込み完了後に実施）:
      ...
      **外部副作用がない場合（HAS_SIDE_EFFECTS = false）:**
        「入力源 → 変換プロセスの連鎖 → 出力先（戻り値受け取り元/後続プロセス）」の形式で描く。
        `_observation-memo.md` の「入力源」セクションを左側に使用する。
        出力先（戻り値を受け取る関数/変数）が観察できた場合はノードとして描く。
        観察できなかった場合は「呼び出し元へ返却（詳細未調査）」ノードを右側に明示する。
        変数に格納する玉突き連鎖が観察できた場合（関数A → 変数X → 関数B 等）はプロセスノードを連鎖させて描く。
        変更対象関数が複数ある場合: 識別子（関数名）ごとに個別プロセスノードとして描く。
          同一出力先（同じ戻り値受け取り元/後続プロセス）の場合はノードを共有してよい。
          全変更対象を 1 つのプロセスノードに集約してはならない（方式比較時に各関数のデータフローが判別できなくなるため）。
      **共通: 入力源が一切観察されなかった場合:**
...
- Section 4: データ仕様・副作用・フロー:
  ...
  - 4.2（必須）: DFD — 外部副作用の有無に関わらず常に Mermaid DFD を生成する。
    入力源（`_observation-memo.md` 「入力源」セクション）を元データに Mermaid graph LR で生成する。
    副作用あり: 「外部エンティティ → 変更対象プロセス → データストア/外部システム」形式。
    副作用なし: 「入力源 → 変換プロセスの連鎖 → 出力先/呼び出し元へ返却」形式（変数玉突き連鎖も可視化）。
