...
上記「適用しない」3ブロックはいずれもTM生成・CRS更新ロジックとは独立したprogress.md書き込み処理
または `/xddp.06.design` 固有の報告文言であり、これを含めて複製すると Step 8 で明記した
「progress.md は更新しない」という設計方針と直接矛盾するか、報告内容が二重になる。一方
`**警告の出力**` ブロック（3種の警告）はTM/CRSの整合性に関する実質的な検証結果であり、
