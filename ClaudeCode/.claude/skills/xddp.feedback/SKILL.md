...
- repo（省略可。プランレビュー8回目 指摘#19対応で挙動を明確化）: 省略時は
...
→ let `AFFECTED_REPOS`（この時点では `REPOS_KEYS` の全コピー。`## Resolve Affected Repos`
  〔`xddp.common/SKILL.md:67-90`〕自体は `repo` を Input として受け取らない設計のため、
  絞り込みは呼び出し元である本スキル側で以下のとおり明示的に行う）。
...
| `design` | Read `~/.claude/skills/xddp.common/SKILL.md`, apply "## Discover CHD Files" with: `CR_PATH: {CR_PATH}, REPO_NAME: {repo}, CR: {CR}` → let `CHD_CONTENT_FILES`（全UR内容ファイル。`xddp.06.design/SKILL.md:386-388` と同一の呼び出し規約） | `{CR_PATH}/06_design/cross/CHD-{CR}-cross.md` |
...
（`xddp.sync-design/SKILL.md:36-46` と同一の取得方法・同一の空diff時ガード（Ask分岐）を適用する）
...
（`xddp.sync-design/SKILL.md:56-67` と同一のロジック。冒頭で `REPO_ARG` により単一repoへ絞り込み
済みの場合はその1repoのみが対象になる）
...
**軽減策（リスク#6対応）:** 変更ファイルとCHD Section 4「変更ファイル」列の突合は、素の文字列一致
ではなくこの正規化後の値で行う。相対パス基点の違いや先頭`./`の有無による偽陰性を防ぐ。
...
For each `{repo}` in `AFFECTED_REPOS`:
  ...
  For each `{cf}` in `CHANGED_FILES_MAP[{repo}]`:
    If `NORMALIZE({cf})` がいずれの `ALL_CHD_FILES` の正規化済みSection 4 集合にも出現しない:
      **フォールバック（軽減策・リスク#6対応）:** `{cf}` のベースファイル名（パス区切りの最後の
...
### Step 1-code-b2: Confirm probable-match candidates（軽減策・リスク#6対応で新設）
...
### Step 1-code-b3: Scale warning（軽減策・リスク#7対応で新設）
...
**軽減策（リスク#5対応）:** 対象ファイルへの直接上書きは行わず、`{file}.pending`
（元のファイル名全体に `.pending` を付与したパス。例: `CHD-{CR}-UR-01.md` → `CHD-{CR}-UR-01.md.pending`）
という一時ステージングファイルに書き込む。`Discover CHD Files`（`xddp.common/SKILL.md:295-316`）は
  ...
  For each `{file}` in `AFFECTED_CHD_FILES_MAP[{repo}]`:
    ...
    **Agent tool** `subagent_type=xddp-chd-sync-agent`:
    ...
    CRS_FILE: {`{CR_PATH}/03_change-requirements/CRS-{CR}.md` if exists, else ""}
      （`xddp.sync-design/SKILL.md:59-60` の `CRS_FILE_MAP` と同一の存在チェック＋空文字
      フォールバック方式。`xddp-chd-sync-agent.md` の Inputs 説明・Process 4「If `CRS_FILE`
      is non-empty: read it」は非空/空の両方を前提とした契約であるため、呼び出し元もこれに
      合わせる）
    ...
    CHANGE_SUMMARY: {REST_ARGSに変更サマリの指定があれば使用。なければGIT_DIFFの変更ファイル一覧
      から自動生成（xddp.sync-design/SKILL.md:48-49 と同一方針）}
...
For each `{repo}` in `AFFECTED_REPOS`, for each `{file}` in `AFFECTED_CHD_FILES_MAP[{repo}]`:
  ...
  Let `FILE_NAME` = basename of `{file}`（例: `CHD-{CR}-UR-01.md`、`CHD-{CR}-UR-01-2.md`。
    `xddp.06.design/SKILL.md:125-127` の `FILE_NAME` 命名規則 `CHD-{CR}-{UR-ID}.md` /
    `CHD-{CR}-{UR-ID}-{N}.md` により生成されたファイル名）
  ...
  （既存のdesignレビューファイルと同一パスを再利用する。`xddp.06.design/SKILL.md:271` と同一の
  命名規則。既存ファイルがあれば追記ラウンドとして扱う）

  **Agent tool** `subagent_type=xddp-reviewer`:
  ```
  DOCUMENT_TYPE: CHD
  TARGET_FILE: {PENDING_FILE}
  REFERENCE_FILES: [{file}（変更前の内容との比較用）, {CR_PATH}/03_change-requirements/CRS-{CR}.md]
  REVIEW_ROUND: {既存レビューファイルがあれば記載されている最終ラウンド+1、なければ1}
...
If 承認:
  For each 承認された `{file}`:
    ...
    （`xddp.06.design/SKILL.md:185-187` と同一の自己申告更新規約を、承認後のこのタイミングで適用する）。
...
If 非承認:
  ...
  Stop.
（`xddp.sync-design/SKILL.md:110-127` と同一のUXパターンを踏襲しつつ、対象ファイル本体への
反映タイミングを承認後に遅らせている点が異なる — リスク#5対応の核心）
...
**DOC_TYPE = test の場合（プランレビュー1回目 指摘#1・#7 対応で範囲を限定）:**
...
本コマンドの対象外とする。「確認項目」（`06_change-design-document-template.md:186` の
Section 7見出し）はCHD固有の概念でありCRSのUSDM構造（UR/SR/SP）に
...
`FEEDBACK_ITEMS` が空の場合（プランレビュー10回目 指摘#21対応。`DOC_TYPE` で分岐する）:
...
- `DOC_TYPE` が `design` または `code` の場合: 「CRSへ反映すべきテキスト形式の未反映項目は
  ...
  Step 3・Step 4・Step 6・Step 7 をスキップし Step 5 へ直接進む（プランレビュー11回目 指摘#23対応：
  「整合性チェックのみ」という表現は Step 5 が実際には TM-{CR}.md の書き出し・CRS Section 3.1 の
  in-place 更新を伴うファイル変更処理であることと用語上ずれるため、「再生成・更新」という Step 5
  見出しと同じ表現に統一する）。これは `xddp.06.design/SKILL.md` Step C'（`L380-462`）が
...
（`xddp.revise/SKILL.md:79` と同じUX）

除外指定があれば `FEEDBACK_ITEMS` から該当項目を除く。残りが空になった場合
（プランレビュー10回目 指摘#21対応。Step 2 の空判定と同一の理由・同一の分岐）:
...
- `DOC_TYPE` が `design` または `code` の場合: 「CRSへ反映するテキスト形式の項目はすべて除外
  ...
  Step 6・Step 7 をスキップし Step 5 へ直接進む（プランレビュー11回目 指摘#24対応：Step 2 側の
  早期終了分岐と同種の状態メッセージを追加し、除外操作の結果として何が続行されるかを利用者に
  明示する。`code` を `design` と同一分岐にする理由はStep 2末尾の注記と同一）。
...
**実行条件（プランレビュー10回目 指摘#21対応。`code` は本改訂で対象に追加）:** `DOC_TYPE = design`
または `code` の場合、このステップは Step 2/Step 3 でテキスト形式の `FEEDBACK_ITEMS` が空になった
場合や Step 4 がスキップされた場合でも必ず実行する（Step 2/Step 3 の分岐を参照）。TM再生成・CHD
未対応SP警告等はCRSへのテキスト反映（Step 4）の成否と無関係に独立して実行される、`xddp.06.design`
Step C' と同じ「無条件実行」の処理だからである。`code` の場合、Step 1-codeでCHD Section 4が
更新されている可能性が高く、TM再生成の実施価値は `design` の場合と同等以上に高い。

`xddp.06.design/SKILL.md:386-436`（Step C' のうち「`For each {repo} in AFFECTED_REPOS:` の
CHD走査ループ」から「CRS TM Section 3.1 の設計・実装列を更新し 変更履歴 に追記する」行まで。
TM-{CR}.md書き出し・CRS Section 3.1 更新ロジックのみで構成される、progress.md への言及を含まない
純粋なTM/CRS処理範囲）と、**`L443-457`（警告の出力ブロックのうち「CHD未対応SPの警告」
「SP間ファイル衝突の警告」「未実装SRの警告」の3種）**を同一の処理として適用する
（プランレビュー7回目 指摘#18対応）。
（重複コード。変更時は両ファイルの該当ブロックを grep して同期させること。
`ARCH_AGENT_PATHS` 等が確立した本リポジトリの `_BASE` 系複製規約に倣う）

**除外範囲（プランレビュー5回目 指摘#10・7回目 指摘#18対応・行単位で再指定。重要）:**
`xddp.06.design/SKILL.md` の以下の行は適用しない:
- `L380-383`（Step見出しと「このステップでは...」というコメント注記のみ。処理指示ではないため
  適用しても実害はないが、対象範囲を明確にするため含めない）
- `L384`（`Update {CR_PATH}/progress.md step 6b 詳細ステップ → Step C': TM生成中.` という
  progress.md書き込み指示。**プランレビュー3回目・4回目で見落とされていた行**）
- `L437-441`（「progress.md の 成果物 列を更新する」ブロック）
- `L442`（空行。区切りのみで処理指示ではないため対象外）
- `L458`（空行。`L457`〔警告出力ブロック終端〕と `L459`〔Tell the user 開始〕の間の区切りのみで
  処理指示ではないため対象外。`L442`と同種の扱い。プランレビュー8回目 指摘#20対応で明示追加）
- `L459-462`（「Tell the user」という `/xddp.06.design` 自身の完了報告。`xddp.feedback` は
  Step 8 で独自の完了報告を行うため、この重複する報告文言は適用しない）

`L380-385`・`L437-442`・`L459-462` はいずれもTM生成・CRS更新ロジックとは独立した
progress.md書き込み処理または `/xddp.06.design` 固有の報告文言であり、これを含めて複製すると
Step 8 で明記した「progress.md は更新しない」という設計方針と直接矛盾するか、報告内容が
二重になる。一方 `L443-457`（3種の警告）はTM/CRSの整合性に関する実質的な検証結果であり、
progress.mdへの書き込みを伴わないため、`xddp.feedback` でも同一に提示する
（`/xddp.06.design` と `/xddp.feedback design` の間で機能パリティを保ち、事後編集によって
生じたCHD未対応SP・SP間衝突・未実装SRを見落とさないようにするため）。
適用範囲は必ず `386-436, 443-457` のみとし、`380-385`・`437-442`・`458`・`459-462` はいずれも
対象外であることを実装時に確認すること。
...
Step 4 で CRS が更新された場合のみ実行する（Step 6 と同一のガード条件。プランレビュー6回目
指摘#17対応：Step 3 で全項目除外され `FEEDBACK_ITEMS` が空になった場合はStep 4自体が
スキップされるため、その場合はStep 6同様Step 7も実行しない）。
...
（プランレビュー5回目 指摘#12対応）: `/xddp.review {CR} req` の `NEXT_DOCUMENT_TYPE: SPO` は
