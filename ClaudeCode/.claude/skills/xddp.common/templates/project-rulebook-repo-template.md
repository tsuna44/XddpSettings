...
> - プロジェクト全体の規約は `project-rulebook.md`（共通）に委ねる
> - クロスリポジトリのインタフェース規約は `project-rulebook-cross.md` に委ねる
>
> **更新ルール:**
> - Section 4（コーディング規約・既存パターン）は**追記型 Upsert**で更新する（同一 `### {テーマ}` があれば既存コードブロック末尾に追記、なければ新規サブセクション追加）
> - Section 6（禁止事項・注意事項）は**置換型 Upsert**で更新する（同一識別キーワードのルールがあれば行群を置換、なければ末尾追記）
> - Section 5（ADR）は追加型で更新する（変更履歴として保持）
> - Section 7（変更履歴）は追加型で更新する（現行通り）
> - 各エントリに更新日と理由を記載する
> - パターン中心で記述する（個別ファイルの列挙ではなく）
> - 1セクション 100〜200 行を目安に保つ
> - 認証情報・APIキーは絶対に記載しない
> - 未記入セクションは `/xddp.fill-rulebook {REPO_NAME}` を実行することで CR の記述内容をもとに自動ドラフトできます
...
> プロジェクト全体の ADR は `project-rulebook.md` に記載する。
