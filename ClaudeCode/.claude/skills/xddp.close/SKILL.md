...
7. **「変更要求仕様書（CRS）ナビゲーション」セクション（upsert）:**
   ...
   For each `{repo}` in `AFFECTED_REPOS`:
     If `{CR_PATH}/03_change-requirements/CRS-{CR}.md` exists:
       Upsert row: "`{CR}` 変更要求仕様 → `{DOCS}/{repo}/crs/CRS-{CR}.md`"
   If `HAS_CROSS` and `{CR_PATH}/03_change-requirements/CRS-{CR}.md` exists:
     Upsert row: "`{CR}` cross 変更要求仕様 → `{DOCS}/cross/crs/CRS-{CR}.md`"
...
For each `{repo}` in `AFFECTED_REPOS`:
  ...
  If `{CR_PATH}/03_change-requirements/CRS-{CR}.md` exists:
    copy to `{CRS_TARGET}/CRS-{CR}.md`

If `HAS_CROSS`:
  Create `{DOCS}/cross/crs/` if absent.
  If `{CR_PATH}/03_change-requirements/CRS-{CR}.md` exists:
    copy to `{DOCS}/cross/crs/CRS-{CR}.md`
...
{for each repo in AFFECTED_REPOS:}
> - {repo}: 仕様書 `{DOCS}/{repo}/specs/` / 変更要求仕様書（CRS） `{DOCS}/{repo}/crs/` / テスト `{DOCS}/{repo}/test/` / 知見 `{DOCS}/{repo}/knowledge/` に昇格
> - {repo}: `project-rulebook-{repo}.md` を `{DOCS}/{repo}/project-rulebook.md` に昇格
{if HAS_CROSS:}
> - cross: 仕様書・変更要求仕様書（CRS）・テスト・知見を `{DOCS}/cross/` に昇格（インタフェース破壊的変更: {あり/なし}）
