...
**Arguments:** $ARGUMENTS
- 引数なし → 全未記入セクションを対象
- セクション番号（例: `1.5`、`2`、`6`）→ 指定セクションのみ（複数指定可: `2 6`）
...
| セクション | プレースホルダー文字列（いずれかが残っていれば未記入） |
|---|---|
...
**§1.5 リポジトリ構成（MULTI_REPO: true のみ）:**
...
---
> **保守メモ:** このファイルを変更した場合は、`ClaudeCode/.claude/commands/xddp.fill-steering.md` の要約も合わせて更新すること。
