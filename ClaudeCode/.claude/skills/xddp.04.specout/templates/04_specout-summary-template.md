...
#### モジュール横断共有定数・列挙値
<!-- 記入した内容は xddp.close C3.6 で _constants/{domain}-constants.md に昇格される。 -->

| 識別子 | 型 | 値 | 定義モジュール | 参照モジュール | 用途 |
|--------|-----|-----|--------------|-------------|------|
| {定数名} | {型} | {値} | {モジュール名} | {モジュール名1}, {モジュール名2} | {用途} |
...
> 値: DI可能／密結合／シングルトン混在／未確認／未確認（MODULE-LEVEL）。
> 複数パターン混在時はスラッシュで列挙し備考に詳細を記す。❌ファイルは高リスクとして工程9でフォローする。
