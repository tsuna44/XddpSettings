...
You are orchestrating **XDDP Step 10 (process step 15) — Generate/Update Latest Specifications**.
...
5. **`{DOCS}/AI_INDEX.md` の存在確認（先行更新のため）:**
   ...
   ※ `{DOCS}` ディレクトリ自体の初回作成は xddp.01.init が担う（xddp.10.specs は作成しない）。

6. Read `{CR_PATH}/progress.md`. Set step 15 → 🔄 進行中, 詳細ステップ → `Step 0: 事前準備中`, today. Write back.
...
- **⚠️ 並行 CR による同時更新リスク（実装コメント）:**
  ...
  同一リポジトリに並行 CR が存在する場合は xddp.10.specs を逐次実行することを推奨する。
  将来的なロック機構は improvement-backlog に記録する（今回スコープ外）。
存在しない場合: `~/.claude/skills/xddp.10.specs/templates/09_overview-architecture-template.md` から初期生成する。
...
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_overview-sequence-template.md`
...
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_overview-dfd-template.md`
...
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_overview-data-model-template.md`
...
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_overview-crud-template.md`

**`overview/deployment.md` の生成/更新:**
SPO サマリーに「ハードウェア構成・デプロイ構成」セクションが存在する場合のみ生成する。
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_overview-deployment-template.md`
...
見出しなし・単一の場合は `main-seq.md` をデフォルトとする。
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_cross-sequence-template.md`
...
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_cross-interface-spec-template.md`

**`cross/interfaces/{interface-kebab}/schema.md` の生成/更新:**
テンプレート: `~/.claude/skills/xddp.10.specs/templates/09_cross-interface-schema-template.md`
...
> {指摘内容の概要}
>
> ⚠️ **並行 CR がある場合は xddp.10.specs を逐次実行してください**（AI_INDEX.md 競合更新リスクおよび overview 共有ファイルの同時更新リスク）。
>
...
```markdown
## 工程15 分割実行メモ
<!-- xddp.10.specs が記録。次回実行時に継続対象を特定するために使用する -->
...
分割実行を選択した場合（このタイミングで「## 工程15 分割実行メモ」を書き込んだ場合）に限り、まず以下を算出する:
...
Step 15 のステータスと `次に実行すべきコマンド` フィールドは、分割実行を選択したか・全体完了したかで以下のように分岐して設定する（`次に実行すべきコマンド` は他工程（`xddp.01.init/SKILL.md`、`xddp.04.specout/SKILL.md`）と同じ「裸コマンド1行」形式に統一する）:
- **分割実行を選択した場合**:
  Step 15 (最新仕様書作成) → 🔄 進行中, 詳細ステップ → `Step DONE: 分割実行中（未処理: {未処理ステップ一覧}）`.
  Set "次に実行すべきコマンド" → `/xddp.10.specs {CR}`
...
「クロスインタフェース一覧」の内容は、`xddp-close-promote-agent.md` 56〜65行目（ユースケース一覧：
テーブル列構成、フロントマターキー `related-modules`〔`module:` ではない〕、リンクパス
`system/specs/use-cases/{usecase-kebab}/description.md`）・72〜78行目（モジュール別最新仕様：テーブル列構成・
リンクパス）・80〜87行目（クロスインタフェース一覧：テーブル列構成・リンクパス）と完全に同一の
テーブル列構成・リンクパス規則・フロントマターキー参照に従うこと。promote-agent 側の定義を正本とし、
xddp.10.specs 側はこれを独自に再定義してはならない。

**「モジュール別最新仕様」セクションの行スコープは `SPECOUT_MODULES` に限定すること（拘束力のある指示）:**
本先行 upsert が「モジュール別最新仕様」セクションに upsert する行（`{repo}/{module}` キー）は、
`AFFECTED_REPOS` に含まれる各 `{repo}` について、Step OV（111行目）で取得済みの `SPECOUT_MODULES`
（`{CR_PATH}/04_specout/{repo}/modules/` 配下のディレクトリ名リスト）に含まれるモジュールのみとする。
これは `xddp-close-promote-agent.md` 73行目「今回 CR で生成・更新した全モジュールの行を upsert」の
...
（`xddp-close-promote-agent.md` 83行目の既存ロジック「IS_MULTI への移行対応: 既存 AI_INDEX.md にセクションが
存在しない状態で IS_MULTI=true となった場合は新規追加する」と同じ契約に従う）。
...
- **分割実行を選択した場合:**
  Tell the user:
  > 🧩 工程15（最新仕様書作成）の一部が完了しました（未処理: {未処理ステップ一覧}）。
...
- **全体完了の場合:**
  ...
  >
  > 並行 CR が存在する場合は xddp.10.specs を逐次実行してください（AI_INDEX.md の競合更新リスク）。
