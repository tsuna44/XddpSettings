...
If `progress.md` に `## CR 中止` セクションが存在する場合:
  ヘッダ行に `🛑 中止済み（{中止日}）` バッジを表示し、`⚡ 次のコマンド:` 行は表示しない
  （中止済み CR は再開を前提としないため）。
  工程進捗テーブル・完了カウントの表示は通常通り行う（中止時点の状態を確認できるようにする）。
...
開始日: {開始日}  最終更新: {最終更新日}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
工程進捗: (show all 15 progress rows (工程1〜11・サブ工程含む) with emoji status, 詳細ステップ, and completion dates)
  - 状態が ⬜ 未着手 / ✅ 完了 の行は 詳細ステップ を省略してよい
  - 状態が 🔄 / 👀 / 🔁 の行は 詳細ステップ を必ず表示する
  - 例: `| 5 | 実装方式検討 | 👀 レビュー待ち | Step B2: 人レビュー待ち | ... |`

完了: {X} / 15 行（工程1〜11・サブ工程含む）
...
🧩 工程11（最新仕様書作成）分割実行中:
  完了済みステップ: {完了済みステップ}
  未処理ステップ: {未処理ステップ一覧}（算出式は `xddp.11.specs/SKILL.md` Step DONE で定義したものと同一。「全体ステップ一覧（UC・OV・MOD、IS_MULTI の場合のみ CROSS も含む）から完了済みステップを除いたもの」を本セクションでも同じ式で再計算する）
  {未処理リポジトリ がある場合} 未処理リポジトリ: {未処理リポジトリ}
  {処理済みモジュール がある場合} 処理済みモジュール: {処理済みモジュール} in {repo}
  {未処理UR がある場合} 未処理UR: {未処理UR}
  ⚡ 継続コマンド: /xddp.11.specs {CR番号}
```
...
```
📦 リポジトリ別進捗:
  工程4a（スペックアウト）: repo-a ✅  repo-b 🔄  cross ⏳
  工程7（コーディング）:   repo-a ✅  repo-b ⏳  cross/検証 ⏳
```
...
Else if step 11 (最新仕様書作成) is ✅ 完了 but クローズ未実施:
