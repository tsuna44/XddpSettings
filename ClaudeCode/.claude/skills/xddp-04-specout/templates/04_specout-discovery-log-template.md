...
> **セル記法:** 本ログのテーブルのセル値に含まれる `|` は、Markdown の列区切りと
...
> ENTRY_POINTS 行のみ `（指定なし）`、他4行は `（なし）`。値ありのセルは各シンボルを
> バッククォートで囲み `, ` で連結する。
...
> `specout_bfs.py` の `cmd_search` が波ごとに upsert する
> （その波に未ヒットが無い場合はセクションごと削除される。
> 波ごとに別セクションとなるため、Wave 0 と再投入波の記録は共存する）。
