...
> モジュールが保有・参照するエンティティ・データ構造体を記録する。
> RDB を持つシステム: ER図（mermaid erDiagram）で記述する。
> 組み込み・非RDBシステム: クラス図またはテキスト形式で記述する。
> `full` レベルのみ生成。ここで生成した内容がサマリー SPO §4.3 のマージ元となる。
