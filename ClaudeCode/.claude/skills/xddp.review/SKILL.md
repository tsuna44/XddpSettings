...
| `specout` | `SPO` | `{repo}` が `"cross"` の場合: `{CR_PATH}/04_specout/cross/SPO-{CR}-cross.md`。それ以外: `{CR_PATH}/04_specout/{repo}/SPO-{CR}.md`（`{repo}` 未指定時: IS_MULTI ならユーザーに確認〔cross も選択肢に含める〕、単一リポジトリなら REPOS_KEYS[0]。NOT IS_MULTI で `{repo}` が `"cross"` の場合はエラー「シングルリポジトリ構成には cross 成果物は存在しません」で停止） | `{repo}` が `"cross"` の場合: `{CR_PATH}/01_requirements/` (all .md), `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, for each `{r}` in REPOS_KEYS: `{CR_PATH}/04_specout/{r}/SPO-{CR}.md` (if exists)。それ以外: `{CR_PATH}/01_requirements/` (all .md), `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, `{CR_PATH}/04_specout/{repo}/modules/` (all *-spo.md), `{CR_PATH}/04_specout/cross/` (all .md, if exists) | `{repo}` が `"cross"` の場合: `{CR_PATH}/04_specout/cross/review/04_specout-cross-review.md`。それ以外: `{CR_PATH}/04_specout/{repo}/review/04_specout-review.md` |
| `arch` | `DSN` | `{repo}` が `"cross"` の場合: `{CR_PATH}/05_architecture/cross/DSN-{CR}-cross.md`（比較案を持たない単一ファイルのため comparison.md/approach-*.md の判定は行わない）。それ以外: TARGET_FILE の決定: `{CR_PATH}/05_architecture/{repo}/DSN-{CR}-comparison.md` が存在する場合（2案以上）: これを TARGET_FILE とする。存在しない場合（1案）: `{CR_PATH}/05_architecture/{repo}/DSN-{CR}-approach-A.md` を TARGET_FILE とする。（`{repo}` が未指定の場合: IS_MULTI ならユーザーに確認〔cross も選択肢に含める〕、単一リポジトリなら REPOS_KEYS[0]。NOT IS_MULTI で `{repo}` が `"cross"` の場合はエラー「シングルリポジトリ構成には cross 成果物は存在しません」で停止） | `{repo}` が `"cross"` の場合: `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, `{CR_PATH}/04_specout/cross/SPO-{CR}-cross.md` (if exists), for each `{r}` in REPOS_KEYS: `{CR_PATH}/05_architecture/{r}/DSN-{CR}-comparison.md`（exists かつ 2案以上の場合）, `{CR_PATH}/05_architecture/{r}/DSN-{CR}-approach-A.md`（exists の場合）, `{CR_PATH}/05_architecture/{r}/DSN-{CR}-approach-B.md`（exists の場合）, `{CR_PATH}/05_architecture/{r}/DSN-{CR}-approach-C.md`（exists の場合）。それ以外: `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, `{CR_PATH}/04_specout/{repo}/SPO-{CR}.md`, `{CR_PATH}/05_architecture/{repo}/DSN-{CR}-approach-A.md` （comparison.md が TARGET の場合）, `{CR_PATH}/05_architecture/{repo}/DSN-{CR}-approach-B.md` （exists の場合）, `{CR_PATH}/05_architecture/{repo}/DSN-{CR}-approach-C.md` （exists の場合） | `{repo}` が `"cross"` の場合: `{CR_PATH}/05_architecture/cross/review/05_architecture-cross-review.md`。それ以外: `{CR_PATH}/05_architecture/{repo}/review/05_architecture-review.md` |
| `design` | `CHD` | 単一バッチファイル（「## 1b. design TARGET_FILE の解決」参照） | `{repo}` が `"cross"` の場合: `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, `{CR_PATH}/04_specout/cross/SPO-{CR}-cross.md` (if exists), `{CR_PATH}/05_architecture/cross/DSN-{CR}-cross.md` (if exists), for each `{r}` in REPOS_KEYS: `{CR_PATH}/06_design/{r}/CHD-{CR}.md` (if exists)。それ以外: `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, `{CR_PATH}/04_specout/{repo}/SPO-{CR}.md` | `{repo}` が `"cross"` の場合: `{CR_PATH}/06_design/cross/review/06_design-cross-review.md`（cross CHD は単一ファイルのため UR-ID/バッチ番号は付与しない）。それ以外: `{CR_PATH}/06_design/{repo}/review/06_design-review-{UR-ID}[-{N}].md` |
| `test` | `TSP` | `{repo}` が `"cross"` の場合: `{CR_PATH}/09_test-spec/cross/TSP-{CR}-cross.md`。それ以外: `{CR_PATH}/09_test-spec/{repo}/TSP-{CR}.md`（`{repo}` 未指定時: IS_MULTI ならユーザーに確認〔cross も選択肢に含める〕、単一リポジトリなら REPOS_KEYS[0]。NOT IS_MULTI で `{repo}` が `"cross"` の場合はエラー「シングルリポジトリ構成には cross 成果物は存在しません」で停止） | `{repo}` が `"cross"` の場合: `{CR_PATH}/06_design/cross/CHD-{CR}-cross.md` (if exists), `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, `{CR_PATH}/04_specout/cross/SPO-{CR}-cross.md` (if exists)。それ以外: `{CR_PATH}/06_design/{repo}/` の CHD内容ファイル全件（「## 1c. test REFERENCE_FILES の解決」参照）, `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, `{CR_PATH}/04_specout/{repo}/SPO-{CR}.md` | `{repo}` が `"cross"` の場合: `{CR_PATH}/09_test-spec/cross/review/09_test-spec-cross-review.md`。それ以外: `{CR_PATH}/09_test-spec/{repo}/review/09_test-spec-review.md` |
...
1. `{repo}` が未指定の場合: IS_MULTI ならユーザーに確認〔cross も選択肢に含める〕、単一リポジトリなら
   REPOS_KEYS[0]。NOT IS_MULTI で `{repo}` が `"cross"` の場合はエラー「シングルリポジトリ構成には
