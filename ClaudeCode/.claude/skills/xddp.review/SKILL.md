...
| `spec` | `SPEC` | latest-specs 配下の単一ファイル（「## 1d. spec TARGET_FILE の解決」参照） | `{CR_PATH}/03_change-requirements/CRS-{CR}.md`, 全 repo の CHD内容ファイル（「## 1d」参照） | `{CR_PATH}/review/09_specs-manual-review/{TARGET_REL}`（TARGET_FILE のパスをそのままミラーするため一意。「## 1d」参照） |
...
5. OUTPUT_FILE の解決: 解決済みの `TARGET_FILE` から `{XDDP_DIR}/latest-specs/` プレフィックス
   ...
   `{CR_PATH}/review/09_specs-manual-review/{TARGET_REL}` とする（ディレクトリ区切り `/` を
