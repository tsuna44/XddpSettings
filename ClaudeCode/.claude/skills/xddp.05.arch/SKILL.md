...
You are orchestrating **XDDP Step 05 (process step 5) — Implementation Approach Design**.
...
→ let `HAS_CROSS`.
（本工程は直前工程＝specout の cross SPO の有無で cross 処理要否を判断する）
...
Let `ARCH_CALL_SHARED` =
  CR_NUMBER: {CR}
  TODAY: {TODAY}
（{repo} に依存しないため、ARCH_TEMPLATE_PATHS と同じ理由で Step A/B/B3 のどの独立ループからも
この1箇所の定義をそのまま参照できる。REPO_NAME はループ変数のため各呼び出し箇所に個別記述のまま残す）
...
1. CRS §2 の SP-ID 一覧を取得する。方法: CRS ファイルを Read して §2 セクション内に絞り、テーブル行（`\|\s*SP-` のパターン — Markdown フォーマッタによる先頭スペース挿入に対応）の第 1 列から識別子を抽出・重複排除する（本文参照・変更履歴等のノイズを排除するため `SP-` プレフィックスを持つセルに限定する）
...
3. Let `WARNINGS` = `[]`（この repo についての警告リスト）。2つの ID 集合を比較し、以下のケースで `WARNINGS` に追加する:
   - CRS にあって funcmap にない SP-ID が存在する場合（工程4bでの SP 追加または funcmap 未収録）:
     「⚠️ funcmap 未収録 SP 項目: {ID一覧}。funcmap は工程4a時点のスナップショットです。これらの SP 項目のシグネチャは CRS §2 を直接照合し、方式比較に組み込んでください。comparison.md「## 4. 採用方式と理由」の懸念事項と対策テーブルに記録してください。」
   - funcmap にあって CRS にない SP-ID が存在する場合（工程4bでの SP 変更・削除による旧 ID 残存）:
     「⚠️ funcmap 余剰エントリ: {ID一覧}。CRS §2 に対応する SP 項目が存在しません。方式比較では CRS §2 の最新 SP 項目を正として扱ってください。」
...
- For each `{repo}` in `AFFECTED_REPOS`: Read `~/.claude/skills/xddp.common/SKILL.md`,
  apply "## Final Review Pass" with:
    DOCUMENT_TYPE: DSN
    NEXT_DOCUMENT_TYPE: CHD
    TARGET_FILE: `{CR_PATH}/05_architecture/{repo}/DSN-{CR}-comparison.md` が存在すれば comparison.md、
      なければ approach-A.md（Step B と同一の判定ロジック）
