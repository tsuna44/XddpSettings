...
> **検証手段タグ（PLAN-20260725-p2-test-harness）:** 下表の確認内容は、いまや3つの手段で守られる。
> - **機械検証（`make test`＝L1〜L3・0トークン）:** 参照整合（`apply` 見出し・`subagent_type`・引数契約・
>   テンプレートプレースホルダー・スクリプト↔スキル結線）と全 unittest。**`make test` が緑なら、これらの
>   静的整合を目視する項目は常時実施リストから省略可。**
> - **full-run 自動（`make smoke-full PHASE=NN`＝L4/L5）:** スキル制御フロー挙動・成果物の構造的健全性
>   （必須セクション・フロントマター・TM 整合・progress の ✅ 到達・未置換トークン残存）。校正ラン完了後に有効。
> - **手動 LLM 検証が必要:** 上記で機械化できない意味的な良し悪し（レビュー判断の妥当性・散文の質など）。
>
> 下表の各行は、まず `make test`（該当すれば `make smoke-full`）を実行し緑を確認したうえで、
> 機械化できない部分のみをフィクスチャで手動確認する運用とする。
...
- **実行コマンド列**:
  ```
  /xddp.01.init CR-2026-910 "モジュール間送信データへの付帯情報追加" xddp-fullrun-req.md
