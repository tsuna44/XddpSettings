...
| `xddp.common/scripts/xddp_metrics.py` | 工程別テレメトリ CLI（`phase-start`／`record`）。`## Snapshot Phase Baseline`〜`## Progress Update`（完了時）間の壁時計所要時間と、`## Review Loop` のレビューラウンド数・収束可否を `{CR_PATH}/metrics.jsonl` へ1行1イベントで追記する | 「## Snapshot Phase Baseline」「## Review Loop」「## Progress Update」 |
